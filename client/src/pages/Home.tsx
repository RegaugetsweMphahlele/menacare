...
const lessonDetails: Record<string, string> = {
  "Starting your period": "Your first periods can arrive with changes in energy, mood, and flow. There is no need to know everything at once: keeping a small pad kit and a trusted adult in mind can make the first days feel less surprising.",
  "Understanding your cycle": "A cycle is the time from the first day of one period to the first day of the next. It can take time to settle into a pattern, especially in the first few years. Logging what is usual for you can make changes easier to notice.",
  "Symptoms & how to manage": "Cramps, tiredness, bloating, and mood changes can happen around a period. Rest, warmth, water, gentle movement, and talking to someone you trust may help. Severe, new, or worrying symptoms deserve support from a health professional.",
  "Period products 101": "Pads, tampons, reusable products, and period underwear can all be useful. The best choice is the one that feels comfortable, available, and right for your body. A trusted adult or health worker can help you compare options.",
  "Sexual & reproductive health": "Your body belongs to you. Respect, consent, privacy, and the ability to ask questions are important parts of health. If something does not feel safe or right, tell a trusted adult or support person.",
  "When to get help": "Reach out when pain is severe, bleeding feels much heavier than usual, your cycle changes suddenly, or you feel worried or unsafe. You deserve to be taken seriously and supported.",
};
...
function getMenaResponse(question: string) {
  const lowered = question.toLowerCase();
  if (lowered.includes("cramp")) return "Cramps are common because the uterus tightens during a period. Gentle warmth, rest, light movement, and talking to a trusted adult can help. If pain is severe, suddenly different, or stops you doing everyday things, a health professional can help.";
  if (lowered.includes("normal") || lowered.includes("regular")) return "Periods can vary in timing, flow, and how they feel—especially in the first few years. Tracking your usual pattern can be helpful. If you are worried about a big change, very heavy bleeding, or severe pain, please tell a trusted adult or health professional.";
  if (lowered.includes("pad") || lowered.includes("product")) return "Pads come in different sizes and absorbencies. Change one whenever it feels full or uncomfortable, and always wash your hands before and after. Your Learn space has a gentle Period Products 101 guide too.";
  if (lowered.includes("school")) return "Starting your period at school can feel like a lot. If you can, speak to a teacher, school nurse, or another trusted adult. The Get Pads space can help you make a small plan for what to ask for.";
  if (lowered.includes("late") || lowered.includes("missed")) return "Cycles can vary, especially when you are younger or going through changes. Keeping a simple note of your dates can help. If you are worried about a big or ongoing change, please speak with a trusted adult or health professional.";
  return "Thanks for sharing that. I can offer general, supportive information and help you think through your next step. If something feels urgent, severe, unsafe, or overwhelming, please reach a trusted adult or local health professional right away.";
}
...
function Onboarding({ onComplete, onBack }: { onComplete: (name: string, ageGroup: string, firstPeriod: string) => void; onBack: () => void }) {
  ...
  return <div className="onboarding"><aside className="onboarding-aside"><button style={{ background: "transparent", padding: 0, textAlign: "left" }} onClick={onBack}><Brand light /></button><div><p className="mc-kicker" style={{ color: "#FF99D8" }}>Your MenaCare space</p><h1 className="mc-display">Here to help you feel more like yourself.</h1><p>A private corner for the questions, changes, and everyday moments that matter.</p></div><div className="onboarding-quote">“Because her future shouldn’t pause.”</div></aside><main className="onboarding-main"><section className="onboarding-panel"><div className="stepper" aria-label={`Step ${step + 1} of 3`}><span className="active" /><span className={step > 0 ? "active" : ""} /><span className={step > 1 ? "active" : ""} /></div>
    ...
    {step === 1 && <><span className="mc-eyebrow"><Heart size={14} /> A little about you</span><h2 className="mc-display">Which age group feels right for you?</h2><p className="mc-copy">This helps us keep your MenaCare space clear, comfortable, and age-appropriate.</p><label className="field-label" htmlFor="age-group">Age group</label><select id="age-group" className="mc-select" value={ageGroup} onChange={event => setAgeGroup(event.target.value)}><option value="" disabled>Select your age group</option><option value="9–12">9–12</option><option value="13–15">13–15</option><option value="16–18">16–18</option><option value="18+">18+</option></select><p className="hero-note"><LockKeyhole size={13} /> Your details stay in this browser.</p></>}
    {step === 2 && <><span className="mc-eyebrow"><CalendarDays size={14} /> One last thing</span><h2 className="mc-display">Have you had your first period yet?</h2><p className="mc-copy">There is no wrong answer. This simply helps us show the most useful starting points.</p><label className="field-label" htmlFor="first-period">Your starting point</label><select id="first-period" className="mc-select" value={firstPeriod} onChange={event => setFirstPeriod(event.target.value)}><option value="" disabled>Choose an answer</option><option value="Yes">Yes, I have already started</option><option value="No">No, I am still waiting</option><option value="Not sure">Not sure, I have questions about it</option></select></>}
...
function LearnView() {
  const [selectedLesson, setSelectedLesson] = useState("Starting your period");
  return <><section className="detail-hero"><div><span className="mc-kicker">Learn at your pace</span><h2 className="mc-display">Real questions. Clear, kind answers.</h2><p>Explore a topic when it feels right. MenaCare is a judgment-free place to learn about your body and your wellbeing.</p></div><img src={assets.learning} alt="Book blooming with health education" /></section><div className="content-grid"><section className="learn-grid">{lessonTopics.map(([title, description, Icon]) => <button className="learn-card" key={title} onClick={() => setSelectedLesson(title)} aria-pressed={selectedLesson === title}><span className="learn-art"><Icon size={23} /></span><span><strong>{title}</strong><small>{description}</small></span><ChevronRight size={16} color="#64007D" /></button>)}</section><aside className="learn-focus"><span className="mc-kicker" style={{ color: "#FF99D8" }}>Selected lesson</span><h3>{selectedLesson}</h3><p>{lessonDetails[selectedLesson]}</p><p style={{ color: "#FF99D8", fontWeight: 800 }}>Choose another card whenever you are ready.</p><img src={assets.youngWoman} alt="Person offering themselves a comforting hug" /></aside></div></>;
}

function ChatView({ messages, sendMessage, onSupport }: { messages: Message[]; sendMessage: (value: string) => void; onSupport: () => void }) {
  ...
  const submit = () => { const question = draft.trim(); if (!question) return; sendMessage(question); setDraft(""); };
  return <><section className="detail-hero"><div><span className="mc-kicker">Ask Mena</span><h2 className="mc-display">A gentle answer can make a big difference.</h2><p>Mena is here with general, supportive information. For urgent or severe symptoms, reach a trusted adult or a local health professional.</p></div><img src={assets.bot} alt="MenaCare assistant" /></section><div className="chat-layout" style={{ marginTop: "1rem" }}><section className="chat-window"><header className="chat-top"><img src={assets.bot} alt="" /><div><strong>Mena, your AI guide</strong><small><span style={{ color: "#64007D" }}>●</span> Here to listen and help</small></div></header><div className="messages" aria-live="polite">{messages.map((message, index) => <div className={`bubble ${message.sender === "user" ? "user" : ""}`} key={`${message.sender}-${index}`}>{message.text}</div>)}</div><form className="chat-compose" onSubmit={event => { event.preventDefault(); submit(); }}><input aria-label="Message Mena" value={draft} onChange={e => setDraft(e.target.value)} placeholder="Ask Mena anything..." /><button className="send-button" aria-label="Send message" type="submit" disabled={!draft.trim()}><Send size={17} /></button></form></section><aside className="chat-side"><section className="prompt-card"><h3>Try asking</h3><div className="prompt-list">{prompts.map(prompt => <button key={prompt} type="button" onClick={() => sendMessage(prompt)}>{prompt} <ChevronRight size={13} style={{ float: "right" }} /></button>)}</div></section><section className="safe-card"><HeartHandshake size={21} color="#64007D" /><h3 style={{ marginTop: ".55rem" }}>Need more support?</h3><p>If you feel unsafe, very unwell, or overwhelmed, please tell a trusted adult, guardian, teacher, or local health professional as soon as you can.</p><a href="#support" onClick={e => { e.preventDefault(); onSupport(); }}>Explore support options <ArrowRight size={13} /></a></section></aside></div></>;
}
...
export default function Home() {
  ...
  const sendMessage = (value: string) => { const question = value.trim(); if (!question) return; const response = getMenaResponse(question); setMessages(current => [...current, { sender: "user", text: question }, { sender: "bot", text: response }]); };
  const renderView = () => { if (view === "home") return <Dashboard name={name} mood={mood} setMood={setMood} setView={setView} />; if (view === "track") return <TrackView symptoms={symptoms} setSymptoms={setSymptoms} mood={mood} onLog={() => { setLogged(true); window.setTimeout(() => setLogged(false), 2600); }} />; if (view === "learn") return <LearnView />; if (view === "chat") return <ChatView messages={messages} sendMessage={sendMessage} onSupport={() => setView("support")} />; if (view === "pads") return <PadsView requested={requested} requestPad={() => setRequested(true)} />; if (view === "support") return <SupportView status={supportStatus} onTalk={() => setSupportStatus("You have opened a gentle conversation plan. Choose one trusted adult, a calm time to talk, and one thing you would like them to know.")} />; return <ProfileView name={name} ageGroup={ageGroup} firstPeriod={firstPeriod} reset={() => { localStorage.removeItem("menacare-profile"); localStorage.removeItem("menacare-checkins"); setHasOnboarded(false); setScreen("landing"); setView("home"); setName("friend"); setAgeGroup(""); setFirstPeriod(""); setMood(""); setSymptoms([]); }} />; };
